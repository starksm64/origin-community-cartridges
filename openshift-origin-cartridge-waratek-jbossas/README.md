# OpenShift Waratek JBossAS Cartridge

## Waratek Behavior
Provides the JBossAS application server on OpenShift using the Waratek cloud vm. Rather than running the application
server in a separate java vm, a java virtual container (JVC) is created inside of the global cloud vm that is running
as a daemon.

### Waratek specific scripts
bin/start-cloudvm.sh
